...
var SafePeriodic = function(timeout, onpoll, options){
  ...
  var self = this;
  this.reschedule();
  //setTimeout(function(){self.reschedule();},10);
};
SafePeriodic.prototype._clearSchedule = function(){
  if(this._schedule) clearTimeout(this._schedule);
  this._schedule = null;
};
SafePeriodic.prototype.reschedule = function(){
  var self = this;
  this._clearSchedule();
  this._schedule = setTimeout(function(){
      self._do();
    }, this.timeout);
};
SafePeriodic.prototype._do = function(){
  var self = this;
  this.onpoll(function(){
      self.reschedule();
  });
};
SafePeriodic.prototype.paused = function(callback){
  var self = this;
  this._clearSchedule();
  if(this.options.onPause) this.options.onPause();
  callback(function(){ self.reschedule()});
};
