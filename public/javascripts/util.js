...
function isArray(value){ return remedialTypeOf(value) === 'array'; }
...
function dateInTz(datestring){
  var offsetFromDateString = function(datestring){
    var offsetRegex = /([+-])(\d\d)(\d\d)$/;
    var matches = datestring.match(offsetRegex);
    if(matches){
      var minutes = parseInt(matches[2], 10) * 60;
  ...
  var serverOffset = offsetFromDateString(datestring);
  if(d.getTimezoneOffset() == serverOffset){
    return d;
  }else{
    var delta = d.getTimezoneOffset() - serverOffset;
...
//utility poller
// takes a timeout in seconds and a callback to perform periodically
// the callback must call its first argument to reschedule the poll
var SafePeriodic = Class.create({
  initialize: function(timeout, onpoll, options){
    this.timeout = (timeout || 10) * 1000;
    this.options = options || {};
    this.onpoll = onpoll || function(){};
    this.reschedule();
  },
  _clearSchedule: function(){
    if(this._schedule) clearTimeout(this._schedule);
    this._schedule = null;
  },
  reschedule: function(){
    this._clearSchedule();
    this._schedule = setTimeout(this._do.bind(this), this.timeout);
  },
  _do: function(){
    this.onpoll(this.reschedule.bind(this));
  },
  
  paused: function(callback){
    this._clearSchedule();
    if(this.options.onPause) this.options.onPause();
    callback(this.reschedule.bind(this));
  }

});


//utility minimal json client
var ClientFactory = Class.create({
  initialize: function(base_url, format){
    this.format = format || 'json'
    if(base_url.indexOf('?') >= 0){
      this.url = base_url.slice(0, base_url.indexOf('?'));
      this.extraParameters = base_url.toQueryParams();
    }else{
      this.url = base_url;
  ...
  _incapacitateRequest: function(req){
    req.evalResponse = function(){};
  },

  invalidateOutstandingRequests: function(){
    this.requests.each(function(req){ this._incapacitateRequest(req);}.bind(this));
  },

  item_url : function(id){ return this.url + "/" + id + '.' + this.format; },
  collection_url : function(id){ return this.url + "." + this.format ; },
  _result: function(transport){
    if(this.format == 'json'){
      return transport.responseJSON;
    }else{
      return tranposrt.responseText;
    }
  },

  _merge_options: function(target, src){
    return Object.extend((target || {}), src);
  },

  _request: function(url, method, callback, options){
    options = options || {};
    if(!callback) callback = function(e){return e;};
    var onComplete, origComplete = options.onComplete;
    if(origComplete){
      onComplete = function(transport){
        this._removeRequest(transport.request);
        return origComplete(transport);
      }.bind(this);
    }else{
      onComplete = function(transport){
        this._removeRequest(transport.request);
